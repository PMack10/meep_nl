...
//cout << "u1u2 npml" << endl;
     if (chi3) {
        ...
        PLOOP_OVER_IVECS(gv, is, ie, i) {
          ...
          realnum gs_2 = (g1[i] + g1[i + s] + g1[i - s1] + g1[i + (s - s1)]) *
                         0.25; // dmpY at X locations (when fc == ex)
          realnum gs_3 =
              (g2[i] + g2[i + s] + g2[i - s2] + g2[i + (s - s2)]) * 0.25; // dmpZ at X locations

          if (u[i] == 0){ // || u1[i] == 0 || u2[i] == 0) { (making all eps values equal for now, can change later)
    ...
    /*       realnum g1s = g1[i] + g1[i + s] + g1[i - s1] + g1[i + (s - s1)];
         ...
         //   cout << "NR" << endl;
            ...
            if (fc == 0) { // Ex
              ...
              realnum seed1 = f[i];
              realnum seed2 = gs_2 * u[i]; // * u1[i];
              realnum seed3 = gs_3 * u[i]; //  * u2[i];
              runNR(seed1, seed2, seed3, &f[i], &dummyF1, &dummyF2, p1, p2,
                    p3); // TODO add explicit eqn soln check to nr
            }
            else if (fc == 1) {                                               // Ey
              ...
              realnum seed1 = gs_3 * u[i];                                    //  * u2[i];
              realnum seed2 = f[i];
              realnum seed3 = gs_2 * u[i]; //   * u1[i];
              ...
              realnum seed1 = gs_2 * u[i];                                    // * u1[i];
              realnum seed2 = gs_3 * u[i];                                    //  * u2[i];
              realnum seed3 = f[i];

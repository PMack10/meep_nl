    ...
    DOCMP { /// added { here to split the two loop macros
      ...
      ///    cout << "Doing linear pre step call" << ft << "  " << ec << "  "<< i<<  endl;
         ...
         ///   cout << "Doing linear pre step call 2"  << endl;
           ...
           //        << (gvs_eh[ft][i].big_corner()).yucky_val(2) << "]" 
           //        << "  big owned c: [" << (gvs_eh[ft][i].big_owned_corner(ec)).yucky_val(0) << ", "
           //        << (gvs_eh[ft][i].big_owned_corner(ec)).yucky_val(1) << ", "
           //        << (gvs_eh[ft][i].big_owned_corner(ec)).yucky_val(2) << "]" << endl;
      ...
      if (false && s->chi2[ez] && ft == E_stuff) { /// if chi2 is non-zero (only z direction checked, but assumes defined for ALL axes) TODO remove false to use this
